    ...
    return (
        <TouchableOpacity className={'flex flex-col items-start w-60 h-80 relative'}>
            ...
            <View className={'flex flex-row items-center bg-white/90 px-3 py-1.5 rounded-full absolute top-5 right-5'}>
                ...
                <Text className={'text-xs font-rubik-bold text-primary-300 ml-1 mt-0.5'}>{rating}</Text>
            </View>
    ...
    return (
        <TouchableOpacity onPress={onPress} className={'flex-1 w-full mt-4 px-3 py-4 rounded-lg bg-white shadow-lg shadow-black-100/70 relative'}>
            <View className={'flex flex-row items-center absolute px-2 top-5 right-5 bg-white/90 rounded-full z-50'}>
                ...
                <Text className={'text-xs font-rubik-bold text-primary-300 ml-0.5 mt-0.5'}>{rating}</Text>
            </View>
            <Image source={{uri: image}} className={'w-full h-40 rounded-lg'} />
            <View className={'flex flex-col mt-2'}>
                <Text className={'text-xl font-rubik-bold text-black-300'} numberOfLines={1}>{name}</Text>
